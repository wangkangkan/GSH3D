...
class RealImgToPatch(object):
    def __init__(self, ray_sampler):
        self.ray_sampler = ray_sampler
        #self.hwf = hwf      # camera intrinsics

    def raw(self, img, H, W, location):
        rgbs = []
        #for img_i in img:
        for i in range(img.shape[0]): 
            pose = torch.eye(4)         # use dummy pose to infer pixel values
            selected_idcs, pixels_i = self.ray_sampler.realimage_sampler(H, W, img[i], location[i])
            if selected_idcs is not None:
                rgbs_i = img[i].flatten(1, 2).t()[selected_idcs]
            else:
                pixels_i = pixels_i.to(img)
                rgbs_i = torch.nn.functional.grid_sample(img[i].unsqueeze(0), 
                                     pixels_i.unsqueeze(0), padding_mode='border', mode='bilinear', align_corners=True)[0]
                rgbs_i = rgbs_i.flatten(1, 2).t()
            rgbs.append(rgbs_i)
    ...
    def __call__(self, img, H, W, location):
       
        pixels = self.ray_sampler.realimage_sampler_batch(H, W, location)

        rgbs = torch.nn.functional.grid_sample(img, 
                                     pixels, padding_mode='border', mode='bilinear', align_corners=True)

        rgbs = rgbs.flatten(2, 3).transpose(1,2)
        
        return rgbs
        
class RaySampler(object):
    def __init__(self, orthographic=False):#N_samples, 
    ...
    def __call__(self, H, W, rays_o, rays_d, ray_start, ray_end, renderclothmask,rendermeshmask,location):
        ...
        select_inds = self.sample_rays_mask(H, W, rendermeshmask,location)
        ...
        return rays_o, rays_d, ray_start, ray_end, renderclothmask, select_inds, hw#torch.stack([rays_o, rays_d])
    ...
    def realimage_sampler_batch(self, H, W, location):
        
        #select_inds = self.sample_rays(H, W)
        select_inds = self.sample_rays_batch(H, W,  location)
...
class FlexGridRaySampler(RaySampler):
    def __init__(self, H_samples, W_samples, random_shift=True, random_scale=True, min_scale=0.25, max_scale=1., scale_anneal=-1,
                 **kwargs):
        #self.N_samples_sqrt = int(sqrt(N_samples))
        self.H_samples = H_samples
        self.W_samples = W_samples
        
        super(FlexGridRaySampler, self).__init__(**kwargs)#self.N_samples_sqrt**2, 
        ...
        self.h, self.w = torch.meshgrid([torch.linspace(-1,1,self.H_samples),
                                         torch.linspace(-1,1,self.W_samples)])
    ...
    def sample_rays_mask(self, H, W, rendermask,location):
        ...
        #specific location
        h_offset = (location[1]-H/2)/(H/2)
        w_offset = (location[0]-W/2)/(W/2)
        ...
        scaleh = self.H_samples/H#torch.Tensor(1).uniform_(0.125, max_scale)#min_scale,0.06
        scalew = self.W_samples/W
        ...
        return torch.cat([w, h], dim=2)
        
    def sample_rays_batch(self, H, W,location):

        #specific location
        h_offset = (location[:,1]-H/2)/(H/2)
        w_offset = (location[:,0]-W/2)/(W/2)

        scaleh = self.H_samples/H
        scalew = self.W_samples/W
        ...
        batchsize = location.shape[0]
        h = h[None].repeat(batchsize,1,1,1)
        w = w[None].repeat(batchsize,1,1,1)

        h += h_offset.unsqueeze(1).unsqueeze(1).unsqueeze(1)
        w += w_offset.unsqueeze(1).unsqueeze(1).unsqueeze(1)
