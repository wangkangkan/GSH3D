...
from models.narf_deepcap import TriPlaneNARF, MLPNARF
import cv2

class TriNARFGenerator(nn.Module):  # tri-plane nerf
    def __init__(self, num_bone=1, parent_id=None, num_bone_param=None, black_background=False):
        ...
        z_dim = 256#config.z_dim
        crop_background = True#config.crop_background

        # if config.nerf_params.origin_location == "center+head":
            # num_bone_param = num_bone
           
        self.nerf = TriPlaneNARF(z_dim=[z_dim * 2, z_dim], num_bone=num_bone,
                                 bone_length=True,
    ...
    def forward(self, frameidx, rawimg, ray_o, ray_d, near, far, mask_at_box, poses, betas, Th, A, joints2D, extrinsics, intrinsic, bone_mask, pose_to_world, bone_length, z=None, inv_intrinsics=None,
        ...
        batchsize = pose_to_world.shape[0]

        #grid, homo_img = self.ray_sampler(self.size, self.size, batchsize), homo_img
        ...
        inv_intrinsics = inv_intrinsics.float().cuda(pose_to_world.device)
        nerf_output = self.nerf(frameidx, rawimg, batchsize, ray_o, ray_d, near, far, mask_at_box, poses, betas, Th, A, joints2D, extrinsics,intrinsic,
                                bone_mask, pose_to_world, inv_intrinsics, z_for_nerf,
                                z_for_neural_render,
                                bone_length,
                                Nc=48,#self.config.nerf_params.Nc,
                                Nf=64,#self.config.nerf_params.Nf,
        ...
        fg_color = nerf_output[-1]#composing super and lowreso img
        # fg_color0 = fg_color.reshape(batchsize, 128, 128, 3)#self.size240, 320
        # fg_color = fg_color0.permute(0,3,1,2).contiguous()
        #fg_mask = fg_mask.reshape(batchsize, 128, 128)#low resolution for nerf rendering

        # fgcolor = fg_color.permute(0,2,3,1)[0].detach().cpu().numpy()
        # cv2.imwrite('test/fgcolor.png', fgcolor*127.5+127.5)
        
        # fullrgb = fg_color.new_zeros([batchsize,240*320,3], dtype=torch.float32)
        # fullmask = fg_color.new_zeros([batchsize,240*320], dtype=torch.float32)
        # fullrgb = fullrgb.view(-1,3)
        # fullmask = fullmask.view(-1,3)
        # fullrgb[humanmask] = fg_color
        # fullmask[humanmask] = fg_mask
        # fullrgb = fullrgb.reshape(batchsize, 240, 320, 3)
        # fullmask = fullmask.reshape(batchsize,  240, 320)
        # fg_color = fullrgb.permute(0,3,1,2).contiguous()
        # fg_mask = fullmask.reshape(batchsize,  240, 320)
        ...
        if return_disparity:
            ...
            disparity = disparity * 3#self.config.nerf_params.coordinate_scale
            return rendered_color, fg_mask, disparity
        if return_bg:
            return fg_color, fg_mask, bg_color 
            
        samplepts_disp = nerf_output[3]
        sdfgradients,offmesh_sdf,meshpts_smpl_sdf,meshpts_cloth_sdf, meshpts_normal,meshptsgradients = nerf_output[4:10]
        return rendered_color, fg_mask, samplepts_disp, sdfgradients,offmesh_sdf,meshpts_smpl_sdf,meshpts_cloth_sdf, meshpts_normal,meshptsgradients#, fine_weights, fine_depth
