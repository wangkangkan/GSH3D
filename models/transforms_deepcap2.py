...
class RealImgToPatch(object):
    ...
    def __call__(self, img, H, W, pose_2d):#, strtag
        rgbs = []
        self.ray_sampler.setting_sampler()
        #for img_i in img:
        for i in range(img.shape[0]): 
            pose = torch.eye(4)         # use dummy pose to infer pixel values
            selected_idcs, pixels_i = self.ray_sampler.realimage_sampler(H, W, img[i], pose_2d[i])#, strtag
            if selected_idcs is not None:
...
class RaySampler(object):
    ...
    def __call__(self, H, W, rays_o, rays_d, ray_start, ray_end, img,pose_2d):#renderclothmask,rendermeshmask
        ...
        select_inds = self.sample_rays_mask(H, W, img.permute(1,2,0)[...,0]*127.5+127.5,pose_2d)#rendermeshmask
    ...
    def realimage_sampler(self, H, W, img, pose_2d):#
        
        #select_inds = self.sample_rays(H, W)
        select_inds = self.sample_rays_mask(H, W, img.permute(1,2,0)[...,0]*127.5+127.5, pose_2d)#
    ...
    def setting_sampler(self):
        
        self.scaleidx = torch.randint(4,(1,))[0]
        self.jointidx = torch.randint(24,(1,))[0]
        
        self.hidx = torch.randint(12,(1,))[0]
        self.widx = torch.randint(6,(1,))[0]#13
...
class FlexGridRaySampler(RaySampler):
    def __init__(self, N_samples, random_shift=True, random_scale=True, min_scale=0.25, max_scale=1., scale_anneal=-1,
        ...
        self.w, self.h = torch.meshgrid([torch.linspace(-1,1,self.N_samples_sqrt*2),
        ...
        self.multiscale = torch.from_numpy(np.array([0.125, 0.25, 0.5, 1.0], dtype = np.float32))
        # self.scaleidx = torch.randint(4,(1,))[0]
        # self.jointidx = torch.randint(24,(1,))[0]
    ...
    def sample_rays_mask(self, H, W, rendermask, pose_2d):#
        ...
        if self.N_samples_sqrt==256:#full resolution
            ...
            # location1 = 64 + 32*self.hidx
            # location0 = 80 + 16*self.widx
            # #specific location
            # # location = pose_2d[self.jointidx, :2] 
            # # h_offset = (location[1]-256)/256
            # # w_offset = (location[0]-128)/128#256
            # h_offset = (location0-128)/128
            # w_offset = (location1-256)/256
            # h_offset = h_offset.to(rendermask)
            # w_offset = w_offset.to(rendermask)
            h_offset = 0
            w_offset = 0
            ...
            scale = self.multiscale[3]#self.scaleidx
            # if self.multiscale[scaleidx]<max_scale:
                # scale = self.multiscale[scaleidx]
            # else:
                # scale = 1
                # h_offset = 0
                # w_offset = 0
