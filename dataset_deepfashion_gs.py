...
class DemoDataset(Dataset):
    ...
    def sample_smpl_param(self, num, device, val=False):
        assert num == 1
        self.counter = torch.randint(self.trans.shape[0],[1])[0]#(self.counter + 1) % self.length
        #print(self.counter)
    ...
    def get_camera_extrinsics(self, num, device, val=False):
        ...
        cy = 512/2.0
        focal = torch.tensor([
                [self.focals[self.counter], 0, cx],
                [0, self.focals[self.counter], cy],
                [0, 0, 1]
            ]).reshape(3,3).to(device)
            
        return self.cam_ex[self.counter].to(device).unsqueeze(0), \
               focal.unsqueeze(0) #self.focals[self.counter].to(device).unsqueeze(0)
...
class DeepFashionDataset(Dataset):
    def __init__(self, path, transform, resolution=(256, 128), nerf_resolution=(128, 64), flist=None,
        ...
        self.nerf_resolution = (nerf_resolution[1], nerf_resolution[0])#int(nerf_resolution[1]/2)
        self.flist = flist
    ...
    def get_camera_extrinsics(self, batch_size, device, val=False):
        ...
        cx = self.nerf_resolution[0]/2.0
        cy = self.nerf_resolution[1]/2.0
        focal = np.array([
                [focal, 0, cx],
                [0, focal, cy],
                [0, 0, 1]
        ...
        if val:
            ...
            #default_focal = torch.Tensor([focal]).view(1, 1).repeat(batch_size, 1).to(device)
            default_focal = torch.Tensor([focal]).view(1,3, 3).repeat(batch_size, 1,1).to(device)
   
        return default_extrinsics, default_focal

    def batch_rigid_transform(self, rot_mats, init_J):
        joints = init_J.reshape(-1, 24, 3, 1)#torch.from_numpy()
    ...
    def __getitem__(self, index):
        ...
        img = np.array(_img.resize(self.resolution, Image.LANCZOS))
        thumb_img = np.array(_img.resize(self.nerf_resolution, Image.LANCZOS))
        seg = np.array(_seg.resize(self.resolution, Image.HAMMING))
        thumb_seg = np.array(_seg.resize(self.nerf_resolution, Image.HAMMING))
