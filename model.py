...
# import neural_renderer as nr_fixgeometry
...
class VolumeRenderDiscriminator(nn.Module):
    def __init__(self, imageresolution):#opt
        super().__init__()
      
        init_size = imageresolution#opt.renderer_spatial_output_dim
...
class VolumeRenderDiscriminator_pg(nn.Module):
    def __init__(self, imageresolution):#opt
        super().__init__()
        ...
        self.input_dim = 3
        self.init_size = init_size/2#divide 2 for pd 

        self.conv1 = CoordConvLayer(self.input_dim, channels[self.init_size], 1)#an init mapping

        self.convblock_pg = VolumeRenderResBlock(channels[self.init_size], channels[self.init_size])
        
        #convs = [VolumeRenderDiscConv2d(self.input_dim, channels[self.init_size], 1, activate=True)]
        convs = []#[VolumeRenderDiscConv2d(channels[self.init_size], channels[self.init_size], 1, activate=True)]
    ...
    def forward(self, input, lod):

        input = self.conv1(input)
        
        if lod==0:#high resolution training
            input = self.convblock_pg(input)

        if lod>0 and lod<1:#transition stage
            
            out_pg = self.convblock_pg(input)
            
            alpha = lod -  np.floor(lod)

            img = torch.nn.functional.avg_pool2d(input, kernel_size=2, stride=2, padding=0)

            input = img * alpha + out_pg * (1 - alpha)
                    
        #if lod==1:#low resolution training
        #    img = torch.nn.functional.avg_pool2d(input, kernel_size=2, stride=2, padding=0)        
