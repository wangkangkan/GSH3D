...
import numpy as np
import torch
from libraries.stylegan2_ada_pytorch.torch_utils import persistence#libraries.custom_stylegan2
from libraries.stylegan2_ada_pytorch.torch_utils.ops import upfirdn2d#libraries.custom_stylegan2
#from training.networks_stylegan2 import DiscriminatorBlock, MappingNetwork, DiscriminatorEpilogue
from libraries.custom_stylegan2.networks_stylegan2 import DiscriminatorBlock, MappingNetwork, DiscriminatorEpilogue
...
class SingleDiscriminator(torch.nn.Module):
    def __init__(self,
        ...
        cur_layer_idx = 0
        print(channels_dict,img_resolution,self.block_resolutions)
...
class DualDiscriminator_our(torch.nn.Module):
    def __init__(self,
        c_dim,                          # Conditioning label (C) dimensionality.
        img_resolution,                 # Input resolution.
        img_channels,                   # Number of input color channels.
        ...
        cur_layer_idx = 0
        print(channels_dict,img_resolution,self.block_resolutions)
        for res in self.block_resolutions:#128
            in_channels = channels_dict[res] if res < img_resolution else 0
            tmp_channels = channels_dict[res]
            out_channels = channels_dict[res // 2]
            use_fp16 = (res >= fp16_resolution)
    ...
    def forward(self, img, c, update_emas=False, **block_kwargs):
        ...
        #img = torch.cat([img['image'], img['image_raw']], 1)
