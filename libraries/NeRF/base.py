...
class NeRFBase(nn.Module):
    def __init__(self, z_dim: Union[int, List[int]] = 256,
                 view_dependent: bool = False, **kwargs):
        super(NeRFBase, self).__init__()
        #assert hasattr(config, "origin_location")

        #self.config = config
        hidden_size = 256#config.hidden_size

        self.origin_location = 'center_fixed'#config.origin_location
        self.coordinate_scale = 3#config.coordinate_scale
        ...
        nffp = 10#self.config.num_frequency_for_position if "num_frequency_for_position" in self.config else 10
        nffo = 4#self.config.num_frequency_for_other if "num_frequency_for_other" in self.config else 4
    ...
    def _forward(self, pose_to_camera, inv_intrinsics,
