...
def coarse_to_fine_sample(model: nn.Module, image_coord: torch.tensor, pose_to_camera: torch.tensor,
    ...
    coarse_density, coarse_color = model.calc_density_and_color_from_camera_coord_v2(coarse_points, pose_to_camera,
                                                                          ray_direction=None,
    ...
    #render color
    coarse_color = coarse_color.reshape(batchsize, 3, -1, Nc)#[:, :, :, :Nc - 1]
    coarse_depth = coarse_depth.reshape(batchsize, 1, -1, Np)[:, :, :, :-1]
    rendered_color = torch.sum(weights * coarse_color, dim=3).squeeze(1)  # B x 3 x n
    rendered_mask = torch.sum(weights, dim=3).reshape(batchsize, -1)  # B x n
    rendered_disparity = torch.sum(weights * 1 / coarse_depth, dim=3).reshape(batchsize, -1)  # B x n
...
def render(model: nn.Module, pose_to_camera: torch.tensor,
    ...
    :param model:
    :param image_coord:image_coord: torch.tensor, 
    :param pose_to_camera:
    :param inv_intrinsics:
    :param render_scale:
    :param Nc:
    ...
    rendered_color, rendered_mask, rendered_disparity, samplepts_disp, sdfgradients,offmesh_sdf,meshpts_smpl_sdf,meshpts_cloth_sdf,meshpts_normal,meshptsgradients,super_lowreso_img = model.image_synthesis_deepcap(Nc,Nf,model_input=model_input)#_geometry
    return rendered_color, rendered_mask, rendered_disparity, samplepts_disp, sdfgradients,offmesh_sdf,meshpts_smpl_sdf, meshpts_cloth_sdf, meshpts_normal,meshptsgradients, super_lowreso_img 
    
def render0(model: nn.Module, image_coord: torch.tensor, pose_to_camera: torch.tensor,
           inv_intrinsics: torch.tensor, render_scale: float = 1, Nc: int = 64,
           Nf: int = 128, semantic_map: bool = False, return_intermediate: bool = False,
           camera_pose: Optional[torch.Tensor] = None,
           model_input: Dict = {}
    ...
    rendered_color, rendered_mask, rendered_disparity = coarse_to_fine_sample(model, image_coord, pose_to_camera,
                                               inv_intrinsics,
                                               near_plane=near_plane, Nc=Nc,
                                               Nf=Nf,
                                               render_scale=render_scale,
