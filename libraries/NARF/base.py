...
class NARFBase(NeRFBase):
    ...
    def forward(self, frameidx, rawimg, batchsize, ray_o, ray_d, near, far, mask_at_box, poses, betas, trans, shift, joints2D, extrinsics,intrinsic,bone_mask,pose_to_world, inv_intrinsics,
                z, z_rend, bone_length,
                render_scale=1, Nc=64, Nf=128, return_intermediate=False,
                truncation_psi=1,
                camera_pose: Optional[torch.Tensor] = None,
        ...
        """
        model_input = {"frameidx": frameidx,"rawimg": rawimg,"ray_o": ray_o, "ray_d": ray_d,"near": near, "far": far, "mask_at_box": mask_at_box,"pose_to_world": pose_to_world,
        "poses": poses, "betas": betas,"trans": trans, "shift": shift,"joints2D": joints2D, "extrinsics": extrinsics,"intrinsic": intrinsic,"bone_mask": bone_mask,"z": z, "z_rend": z_rend, "bone_length": bone_length, "truncation_psi": truncation_psi}
