...
def requires_grad1(model, flag=True):
    for name, p in model.named_parameters():
        if name not in ['net.sugar_model_smpl._xyz_idx','net.sugar_model_cloth._xyz_idx']:
            p.requires_grad = flag

def set_emagen(model1, model2):
    par1 = dict(model1.named_parameters())
    par2 = dict(model2.named_parameters())

    for k in par1.keys():
        par1[k].data = par2[k].data.clone()
...
def accumulate1(model1, model2, decay=0.999, desifytag=False):
    par1 = dict(model1.named_parameters())
    par2 = dict(model2.named_parameters())

    for k in par1.keys():
        if k in ['net.sugar_model_smpl._xyz_idx','net.sugar_model_cloth._xyz_idx','net.sugar_model_smpl._xyz_weight','net.sugar_model_cloth._xyz_weight'] and desifytag:
            par1[k].data = par2[k].data
        else:
            if k in ['net.sugar_model_smpl._xyz_idx','net.sugar_model_cloth._xyz_idx']:
                par1[k].data = par2[k].data
            else:
                par1[k].data.mul_(decay).add_(par2[k].data, alpha=1 - decay)
