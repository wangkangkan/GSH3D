...
class Dataset(data.Dataset):
    def __init__(self, data_root, human, ann_file, split):
        ...
        self.rawcam_inds = np.array([
            np.array(cfg.raw_view)[cfg.training_view]
            for ims_data in annots['ims'][i:i + ni][::i_intv]
        ]).ravel()
    ...
    def prepare_input(self, i):
        ...
        # DEBUG: 使用模板空间测试
        betas = torch.from_numpy(params['betas'][i].reshape(-1))
        ...
        return coord, out_sh, can_bounds, bounds, Rh, Th, vert, A, np.zeros_like(params['full_pose'][i].reshape(-1)), \
            np.zeros_like(params['betas'][i].reshape(-1))
    ...
    def get_mask(self, i, nv):
        ...
        # msk_path = os.path.join('../../deepcap_dataset/Magdalena/training', 'foregroundSegmentation') + im[6:]
        msk_path = os.path.join('tools/data/dnarendering/0080_09_annots', 'mask', im)
    ...
    def __getitem__(self, index):
        ...
        # R = RT[:3, :3]
        # T = RT[:3, 3:]
        # # r_mat = torch.linalg.inv(R)
        # # # NOTE: right mm to left mm
        # r_mat = R.transpose(-1, -2)
        # # R *= torch.tensor([-1.0, -1.0, 1.0]).to(R.device)
        ...
        pytorch3d_K = self.convert_screen_to_ndc(K=pytorch3d_K, resolution=(cfg.H, cfg.W), index=index, cam_id=cam_ind)[0]
