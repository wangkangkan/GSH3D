...
class Renderer:
    def __init__(self, net:GNS):
        ...
        #self.meshrenderer = meshrendererself.net.device#
        ...
        # canpoints_gs = self.net.sugar_model_smpl.getbatch_edited_points(self.canoncalmeansmpl)
        # np.savetxt('canmeangs.txt',canpoints_gs[0].detach().cpu().numpy())
        # mesh_test = trimesh.Trimesh(vertices=self.canoncalmeansmpl[0].detach().cpu().numpy(), faces=self.smplfaces[0].detach().cpu().numpy(), process=False)
        ...
        self.headmask = self.headmask/255
    ...
    def render_image_gaussian_rasterizer(
        ...
        rendered_image, radii = rasterizer(
            ...
            cov3D_precomp = cov3D)
        if RETRUNMASK:
            splat_mask = torch.zeros_like(splat_colors)
    ...
    def edit_Gaussianattributes(self, gs_incpos, gs_rot, gs_scale, gs_color, gs_opacity):
        ...
        #self.editgsidx = gssegidx==1#lower body
        # self.gs_incpos_smpl[:,editgsidx] = gs_incpos[:,editgsidx]
    ...
    def batch_editfeature(self, styles, cam_poses, focals, beta, theta, trans, gs_incpos, gs_rot, gs_scale, gs_color, gs_opacity):
        ...
        #self.gs_incpos_smpl[:,self.editgsidx] = gs_incpos[:,self.editgsidx]
        
        self.gs_opacity_smpl[:,self.editgsidx] = gs_opacity[:,self.editgsidx]

        self.gs_shs_smpl[:,self.editgsidx] = gs_color[:,self.editgsidx]

        # 预测旋转和尺度(SMPL和CLOTH)
        #self.rotations_smpl[:,self.editgsidx] = gs_rot[:,self.editgsidx]
        self.gs_scales_smpl[:,self.editgsidx] = gs_scale[:,self.editgsidx]
        ...
        points_gs = self.net.sugar_model_smpl.getbatch_edited_points(self.canoncalsmpl)
        self.smplgs = points_gs
        self.canonpersongs = points_gs + self.gs_incpos_smpl
    ...
    def batch_generatefeature(self, styles, cam_poses, focals, beta, theta, trans):
        ...
        # canoncalmeansmpl = so['vertices'].clone().reshape(-1, 6890, 3)#mean smpl
        batchsize = styles.shape[0]
        #print(beta.device,self.thetazero.device)
        # smpl_model = init_smpl(
            # model_folder = 'smpl_models',
            # model_type = 'smpl',
            # gender = 'neutral',#
            # num_betas = 10,
        ...
        # self.incbwloss = 10.0*torch.nn.functional.smooth_l1_loss(self.net.position_enc_smpl.plane_bw, torch.zeros_like(self.net.position_enc_smpl.plane_bw),reduction='sum')#.mean()
        # self.bwtv_loss = self.tv_loss(self.net.position_enc_smpl.plane_bw, 0.1)
        # plane_pos = self.net.position_enc_smpl.plane_pos[0].permute(1,2,0).detach().cpu().numpy()
        # cv2.imwrite('plane_pos.png', (0.5 * plane_pos*10 + 0.5)*255)
        # plane_rot = self.net.position_enc_smpl.plane_rot[0].permute(1,2,0).detach().cpu().numpy()
        # cv2.imwrite('plane_rot.png', np.abs(plane_rot)*50*255) #(0.5 * plane_rot*1000 + 0.5) 
        # # plane_scale = self.net.position_enc_smpl.plane_scale
        # # plane_scale = torch.cat([plane_scale,plane_scale[:,0:1]],dim=1)
        # # plane_scale = plane_scale[0].permute(1,2,0).detach().cpu().numpy()
        # # cv2.imwrite('plane_scale.png', plane_scale*100*255)
        # plane_color = self.net.position_enc_smpl.plane_color[0].permute(1,2,0).detach().cpu().numpy()
        # plane_color = cv2.cvtColor(plane_color, cv2.COLOR_RGB2BGR)
        # cv2.imwrite('plane_color.png', plane_color)
        # plane_opacity = self.net.position_enc_smpl.plane_opacity[0].permute(1,2,0).detach().cpu().numpy()
        # cv2.imwrite('plane_opacity.png', plane_opacity*255)
    ...
    def gsrendering(self, gradtag, bidx, cam_poses, focals):
        ...
        # quaternions_smpl, gs_scales_smpl, gs_quat = self.net.sugar_model_smpl.get_posed_quaternions_and_scales2(canpoints=canpersonalsmpl,
                                                                                                      ...
                                                                                                      # sca=gs_scales_smpl)
        # if bidx==0:
            # pytorch3d_K1 = set_pytorch3d_intrinsic_matrix(projection_matrix, self.resH, self.resH)

            # cameras1 = PerspectiveCameras(device='cuda',
                                             # K=pytorch3d_K1,
                                             # R=mat_R,
                                             # T=mat_T)
            # trans = torch.Tensor([0.6, -0.4, 14]).to('cuda')
            # self.canonpersongs[bidx][:,1:3] = -self.canonpersongs[bidx][:,1:3]
            # self.smplgs[bidx][:,1:3] = -self.smplgs[bidx][:,1:3]
            # self.rendercolorgs(self.canonpersongs[bidx]+trans, self.gs_shs_smpl[bidx], cameras1, 'offsetgs.png')
            # self.rendercolorgs(self.smplgs[bidx]+trans, self.gs_shs_smpl[bidx], cameras1, 'smplgs.png')
            # self.rendercolorgs(self.posedgs[bidx], self.gs_shs_smpl[bidx], cameras1, 'posedgs.png')
            
            # plane_scale = self.net.position_enc_smpl.plane_scale[bidx]            
            # plane_scale = torch.cat([plane_scale,plane_scale[0:1]],dim=0)
            # uvgs = self.net.sugar_model_smpl.UV_GS*256
            # uvgs = uvgs.int()
            # for uvidx in range(0, uvgs.shape[0]):
                # plane_scale[:2,uvgs[uvidx,1],uvgs[uvidx,0]] = gs_scales_smpl[uvidx,1:3]
                # plane_scale[2,uvgs[uvidx,1],uvgs[uvidx,0]] = gs_scales_smpl[uvidx,2]
                
            # plane_scale = plane_scale.permute(1,2,0).detach().cpu().numpy()
            # cv2.imwrite('plane_scale.png', plane_scale*100*255)
        ...
        render_smpl_output = self.render_image_gaussian_rasterizer_depth(
            sh_deg=4,
            quaternions=None,
            return_2d_radii=True,
            return_colors=True,
        ...
        if 0:
            ...
            #rendernormalimg = rendered_normal.permute(1,2,0).detach().cpu().numpy()
            #rendernormalimg = cv2.cvtColor(rendernormalimg, cv2.COLOR_RGB2BGR)
            ...
            self.rendermask = render_smpl_output["mask"].permute(1,2,0)
        ...
        # if bidx==0:
            ...
            # renderimg = cv2.cvtColor(renderimg, cv2.COLOR_RGB2BGR)
            # cv2.imwrite('renderimg.png', renderimg)
        ...
        if RETRUNMASK:
            rendered_mask = render_smpl_output["mask"][0:1].permute(1,2,0)
            rendered_mask = 1-rendered_mask/255
            # renderimg = rendered_mask.detach().cpu().numpy()
            # cv2.imwrite('rendered_mask.png', renderimg*255)
            rendered_mask = rendered_mask[None]
            rendered_img = torch.cat([rendered_img, rendered_mask], dim=-1)

        #rendermask = render_smpl_output["mask"].permute(1,2,0).detach()
        #rendered_normal = render_smpl_output["normal"].permute(1,2,0)
        #rendermask = rendermask/255#1-rendermask/255
        #rendermask = rendermask.repeat(1,1,3).bool()
        #rendered_normal[rendermask] = rendermask[rendermask].float()
        #rendered_img = torch.cat([rendered_img, rendered_normal[None]], dim=-1)
    ...
    def rendercolorgs(self, points_smpl, gs_shs_smpl, cameras, filename):
        ...
        colored_points = struct.Pointclouds(points=[points_smpl], features=[gs_shs_smpl])
        ...
        render_img = renderer(colored_points, cameras=cameras)
        
        cv2.imwrite(filename,cv2.cvtColor(render_img[0].detach().cpu().numpy(), cv2.COLOR_RGB2BGR))
