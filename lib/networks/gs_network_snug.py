...
from lib.networks.modules.decoders import AppearanceDecoder, GeometryDecoder, PositionEncoder, AppearanceDecoder2, GeometryDecoder2
...
class Network(nn.Module):
    def __init__(self):
        ...
        #     num_train_frame=cfg.num_train_frame
        # )
        # self.appearance_dec_smpl = AppearanceDecoder(n_features=cfg.encoder.n_features*2)
        # self.appearance_dec_cloth = AppearanceDecoder(n_features=cfg.encoder.n_features*2)
        self.appearance_dec_smpl = AppearanceDecoder2(n_features=n_features*3)
        #self.appearance_dec_cloth = AppearanceDecoder2(n_features=n_features*3)
        ...
        # self.geometry_dec_smpl = GeometryDecoder(n_features=cfg.encoder.n_features*2, use_surface=True)
        # self.geometry_dec_cloth = GeometryDecoder(n_features=cfg.encoder.n_features*2, use_surface=True)
        self.geometry_dec_smpl = GeometryDecoder2(n_features=n_features*3)
        #self.geometry_dec_cloth = GeometryDecoder2(n_features=n_features*3)
        ...
        cloth_obj = "../gaussian-nerfcap/data/magdalena/magdalena2000-allviews/templatedeformT/cloth/clothes.obj"
        #smpl_obj = "../gaussian-nerfcap/data/magdalena/magdalena2000-allviews/templatedeformT/desmpl/detailsmpl.obj"
        #o3d_mesh_smpl = o3d.io.read_triangle_mesh(smpl_obj)
        o3d_mesh_cloth = o3d.io.read_triangle_mesh(cloth_obj)
        
        smpl_obj = "../gaussian-nerfcap/data/magdalena/magdalena2000-allviews/templatedeformT/smpl/template.obj"
        o3d_mesh_smpl = o3d.io.read_triangle_mesh(smpl_obj)
        ...
        self.desmplfaces = torch.LongTensor(npfaces).to(self.device)
        templatesmpl_path = os.path.join(data_root,
                                         'templatedeformT/smpl/templateshape.txt') 
        templatesmpl = np.loadtxt(templatesmpl_path)
        self.templatesmpl = torch.Tensor(templatesmpl).to(self.device)
        ...
        # TODO: 使用Snug模拟服装
        # from lib.networks.snug_simualtor import NetworkWrapper
        # self.cloth_simulation_snug = NetworkWrapper(cfg.snug)
        # self.vert_displace_network =  VertDispalceNetwork()
        
        # TODO: 使用材质参数
        # self.tempclothpara_snug = nn.Embedding(1, 4)
        # self.tempclothpara_snug = nn.Embedding.from_pretrained(pretrained_model['net']['tempclothpara_snug.weight'], freeze=True)
        # self.tempclothpara = nn.Embedding.from_pretrained(pretrained_model['net']['tempclothpara.weight'], freeze=True)
...
class VaryingClothDeformationNetwork(nn.Module):
    def __init__(self):
        ...
        npfaces = np.loadtxt(os.path.join(data_root, 'templatedeformT/smpl_vfidx.txt')) - 1
...
class ClothSimulation(nn.Module):
    def __init__(self):
        ...
        data_root = '../gaussian-nerfcap/data/magdalena/magdalena2000-allviews'
        templatecloth_path = os.path.join(data_root, 'templatedeformT/cloth/clothes_vert.txt')
