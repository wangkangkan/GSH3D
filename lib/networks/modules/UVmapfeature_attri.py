...
class UVmap_GS(nn.Module):
    def __init__(self, features=32, resX=256, resY=256, resZ=256):
        ...
        self.appearance_gen = prepare_triplane_generator(512, 512, 4,0)#color+opacity
        self.geometry_gen = prepare_triplane_generator(512, 512, 8,0)#incpos+rot+scale
        
        #self.poslinear  = nn.Linear(3, 3)
        self.poslinear = nn.Conv2d(in_channels=3, out_channels=3, kernel_size=3, stride=1, padding=1)        
        torch.nn.init.constant(self.poslinear.weight, 0)
        torch.nn.init.constant(self.poslinear.bias, 0)
        
        #self.rotlinear  = nn.Linear(3, 3)
        self.rotlinear = nn.Conv2d(in_channels=3, out_channels=3, kernel_size=3, stride=1, padding=1) 
        torch.nn.init.constant(self.rotlinear.weight, 0)
        torch.nn.init.constant(self.rotlinear.bias, 0)
        
        self.scalelinear = nn.Conv2d(in_channels=2, out_channels=2, kernel_size=3, stride=1, padding=1) 
    ...
    def uvgsfeature(self, styles):
        ...
        appearance_planes, _ = self.appearance_gen(styles,0,truncation_psi=1)        
        appearance_planes = appearance_planes.view(len(appearance_planes), 4, appearance_planes.shape[-2], appearance_planes.shape[-1])
        
        geometry_planes, _ = self.geometry_gen(styles,0,truncation_psi=1)        
        geometry_planes = geometry_planes.view(len(geometry_planes), 8, geometry_planes.shape[-2], geometry_planes.shape[-1])
        
        
        self.plane_pos = geometry_planes[:,0:3] 
        self.plane_rot = geometry_planes[:,3:6]
        self.plane_scale = geometry_planes[:,6:8]
        self.plane_color = appearance_planes[:,0:3]
        self.plane_opacity = appearance_planes[:,3:4]
