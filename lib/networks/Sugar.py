...
class SuGaR(nn.Module):
    ...
    def __init__(
        ...
        *args, **kwargs) -> None:
        ...
        if surface_mesh_to_bind is not None:
            ...
            self._vertex_colors = torch.tensor(np.array(surface_mesh_to_bind.vertex_colors)).float().to(nerfmodel.device) # NOTE: 初始coarse mesh是有颜色的
    ...
    def get_points_rgb(
        ...
        if camera_centers is not None:
            render_directions = torch.nn.functional.normalize(positions - camera_centers, dim=-1) # 点到相机中心的方向向量
        ...
        if sh_levels is None:
            ...
            sh_coordinates = sh_coordinates[:, :sh_levels**2]  # WARMUP
    ...
    def render_image_gaussian_rasterizer(
        ...
        world_view_transform = torch.Tensor(getWorld2View(  # 齐次坐标 w2c的转置
            R=R, t=T, tensor=use_torch)).transpose(0, 1).cuda()
        
        proj_transform = getProjectionMatrix( # 投影矩阵
        ...
        proj_transform[..., 2, 0] = - p3d_camera.K[0, 0, 2] # DEBUG: 为什么要换？
