...
def load_model(net,
    ...
    net.sugar_model_smpl._xyz_weight = nn.Parameter(pretrained_model['net']['sugar_model_smpl._xyz_weight'].to("cuda:0"), requires_grad=True)
    net.sugar_model_smpl._xyz_idx = nn.Parameter(pretrained_model['net']['sugar_model_smpl._xyz_idx'].to("cuda:0"), requires_grad=False)
    net.sugar_model_cloth._xyz_weight = nn.Parameter(pretrained_model['net']['sugar_model_cloth._xyz_weight'].to("cuda:0"), requires_grad=True)
    net.sugar_model_cloth._xyz_idx = nn.Parameter(pretrained_model['net']['sugar_model_cloth._xyz_idx'].to("cuda:0"), requires_grad=False)
    net.sugar_model_smpl.max_radii2D = torch.zeros((net.sugar_model_smpl._xyz_weight.shape[0]), device="cuda:0")
    net.sugar_model_smpl.xyz_gradient_accum = torch.zeros((net.sugar_model_smpl._xyz_weight.shape[0], 1), device="cuda:0")
    net.sugar_model_smpl.denom = torch.zeros((net.sugar_model_smpl._xyz_weight.shape[0], 1), device="cuda:0")
    net.sugar_model_cloth.max_radii2D = torch.zeros((net.sugar_model_cloth._xyz_weight.shape[0]), device="cuda:0")
    net.sugar_model_cloth.xyz_gradient_accum = torch.zeros((net.sugar_model_cloth._xyz_weight.shape[0], 1), device="cuda:0")
    net.sugar_model_cloth.denom = torch.zeros((net.sugar_model_cloth._xyz_weight.shape[0], 1), device="cuda:0")
    net.load_state_dict(pretrained_model['net'], strict=False)
    optim.load_state_dict(pretrained_model['optim'])
    for param in optim.param_groups:
        if param["name"] == "xyz_smpl":
            param["params"] = [net.sugar_model_smpl._xyz_weight]
        if param["name"] == "xyz_cloth":
            param["params"] = [net.sugar_model_cloth._xyz_weight]
...
def load_network(net, model_dir, resume=True, epoch=-1, strict=True):
    ...
    net.sugar_model_smpl._xyz_weight = nn.Parameter(pretrained_model['net']['sugar_model_smpl._xyz_weight'], requires_grad=True)
    net.sugar_model_smpl._xyz_idx = nn.Parameter(pretrained_model['net']['sugar_model_smpl._xyz_idx'], requires_grad=False)
    net.sugar_model_cloth._xyz_weight = nn.Parameter(pretrained_model['net']['sugar_model_cloth._xyz_weight'], requires_grad=True)
    net.sugar_model_cloth._xyz_idx = nn.Parameter(pretrained_model['net']['sugar_model_cloth._xyz_idx'], requires_grad=False)
