...
class Evaluator:
    def __init__(self):
        ...
        # self.allerrmse = np.ones((48,300),dtype=np.float64) 
        # self.allerrpsnr = np.ones((48,300),dtype=np.float64) 
        # self.allerrssim = np.ones((48,300),dtype=np.float64) 
        # self.boundbox = np.ones((14*300,4),dtype=np.float64) 
    ...
    def ssim_metric(self, img_pred, img_gt, batch):
        if not cfg.eval_whole_img:
            ...
            mask_at_box = batch['msk'][0].detach().cpu().numpy() * 255
        ...
        # cv2.imwrite(
        #     '{}/frame{:04d}_view{:04d}.png'.format(path, frame_index,
        #                                            view_index),
        #     (img_pred[..., [2, 1, 0]] * 255))
        cv2.imwrite(
            '{}/frame{:04d}_view{:04d}.png'.format(path, frame_index,
        ...
        os.makedirs(path, exist_ok=True)    
        cv2.imwrite(
            '{}/frame{:04d}_view{:04d}_gt.png'.format(path, frame_index,
                                                   view_index),
            (img_gt[..., [2, 1, 0]] * 255))
    ...
    def evaluate(self, output, batch):
        ...
        rgb_pred = output['image'].detach().cpu().numpy()
        # rgb_gt = batch['rgb'][0].detach().cpu().numpy()
        rgb_gt = batch['img'][0].detach().cpu().numpy()

        # mask_at_box = batch['mask_at_box'][0].detach().cpu().numpy()
        mask_at_box = batch['msk'][0].detach().cpu().numpy()
        H, W = int(cfg.H * cfg.ratio), int(cfg.W * cfg.ratio)
        mask_at_box = mask_at_box.reshape(H, W)
        ...
        img_pred = rgb_pred #* batch['msk'][0].unsqueeze(-1).detach().cpu().numpy()
        img_gt = rgb_gt * batch['msk'][0].unsqueeze(-1).detach().cpu().numpy()
        
        # img_gt = img_gt*completerendermask[...,None][0].detach().cpu().numpy()
        # img_pred = img_pred*completerendermask[...,None][0].detach().cpu().numpy()
        # rgb_pred = img_pred[mask_at_box], completerendermask
        # rgb_gt = img_gt[mask_at_box]
        ...
        # self.allerrmse[view_index, frame_index] = mse
        # self.allerrpsnr[view_index, frame_index] = psnr       
        # self.allerrssim[view_index, frame_index] = ssim
        print(view_index,frame_index)
        print(mse, psnr, ssim)
    ...
    def summarize(self):
        ...
        self.psnr = []
        self.ssim = []
        np.savetxt(os.path.join(cfg.result_dir, 'allerrmse_our0-150_700.txt'),self.allerrmse)
        np.savetxt(os.path.join(cfg.result_dir, 'allerrpsnr_our0-150_700.txt'),self.allerrpsnr)
        np.savetxt(os.path.join(cfg.result_dir, 'allerrssim_our0-150_700.txt'),self.allerrssim)
        # np.savetxt(os.path.join(cfg.result_dir, 'allboundbox_700.txt'),self.boundbox)
