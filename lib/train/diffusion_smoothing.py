...
def numpy_laplacian_uniform(v, f):
    ...
    clothedge_path = os.path.join('tools/data/magdalena2000-allviews/templatedeformT/cloth/edge.txt')
    clothedge = np.loadtxt(clothedge_path)-1
    value = np.ones_like(clothedge[:,0])
    connectivity = sp.coo_matrix(value, (clothedge[:,0],clothedge[:,1]), shape=[vtnum,vtnum])
    #double edge
