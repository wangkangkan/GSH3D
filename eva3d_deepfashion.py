...
#from op import grid_sample_gradfix
...
class VoxelHuman(nn.Module):
    def __init__(self, opt, smpl_cfgs, style_dim, out_im_res=(128, 64), mode='train'):
        ...
        for j in range(num_joints):
            ...
            if xyz_min is None:
                continue
            xyz_min -= np.array([0.035, 0.035, 0.035])#0.035
    ...
    def predefined_bbox(self, j, only_cur_index=False):
        if j == 15:
            xyz_min = np.array([-0.0901, 0.2876, -0.0891])#0.2876
            ...
            xyz_max = np.array([0.1724, 0.2876, 0.1391])#0.2876
    ...
    def get_rays0(self, K, c2w):
        # dirs = torch.stack([(self.i - self.out_im_res[1] * .5) / focal[0],
                            # (self.j - self.out_im_res[0] * .5) / focal[0],
                            # torch.ones_like(self.i).expand(1,self.out_im_res[0], self.out_im_res[1])], -1).repeat(focal.shape[0], 1, 1, 1)

        dirs = torch.stack([(self.i - K[0,0,2]) / K[0,0,0],
                            (self.j - K[0,1,2]) / K[0,1,1],
                            torch.ones_like(self.i).expand(1,self.out_im_res[0], self.out_im_res[1])], -1).repeat(K.shape[0], 1, 1, 1)
        ...
        for i_chunk in range(len(split_pts)):
            ...
            for i, cur_vox in enumerate(self.vox_list):
                with torch.no_grad():
                    ...
                    # # template_sdf = grid_sample_gradfix.grid_sample(
                        # # self.sdf_voxels, query_pts
                    # # ).reshape(-1)
    ...
    def forward(self, cam_poses, focals, beta, theta, trans, styles=None, return_eikonal=False, no_white_bg=False, fix_viewdir=False, w_space=False, gamma_list=None, beta_list=None):
        ...
        for i in range(len(self.vox_list)):
            ...
            # # template_sdf = grid_sample_gradfix.grid_sample(
                # # self.sdf_voxels[0], cur_xyz.reshape(1, 1, -1, 3) / 1.3
            # # ).reshape(-1, 1)
